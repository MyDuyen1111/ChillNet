...
public class GroupService {
    ...
    public GroupResponse uploadGroupAvatar(String groupId, MultipartFile file) {
        ...
        try {
            var uploadedEvent = imageUploadService.uploadGroupAvatar(file, groupId);
            group.setAvatarUrl(uploadedEvent.imageUrl());
        } catch (Exception e) {
            log.error("Failed to upload group avatar via file-service: {}", e.getMessage(), e);
            throw new AppException(ErrorCode.UNCATEGORIZED_EXCEPTION);
        }
    ...
    public GroupResponse uploadGroupCover(String groupId, MultipartFile file) {
        ...
        // Upload file qua file-service (Feign)
        try {
            var uploadedEvent = imageUploadService.uploadGroupCover(file, groupId);
            group.setCoverImageUrl(uploadedEvent.imageUrl());
        } catch (Exception e) {
            log.error("Failed to upload group cover via file-service: {}", e.getMessage(), e);
            throw new AppException(ErrorCode.UNCATEGORIZED_EXCEPTION);
        }
    ...
    public PageResponse<GroupResponse> getJoinedGroups(int page, int size) {
        ...
        List<String> groupIds =
                pageData.getContent().stream().map(GroupMember::getGroupId).toList();
    ...
    public PageResponse<JoinRequestResponse> getJoinRequests(String groupId, int page, int size) {
        ...
        Pageable pageable =
                PageRequest.of(page - 1, size, Sort.by("requestedDate").descending());
    ...
    private GroupResponse buildGroupResponse(Group group, String currentUserId) {
        ...
        if (isAdminOrModerator(group.getId(), currentUserId)
                || group.getOwnerId().equals(currentUserId)) {
            long pendingRequestCount =
                    joinRequestRepository.countByGroupIdAndStatus(group.getId(), RequestStatus.PENDING);
